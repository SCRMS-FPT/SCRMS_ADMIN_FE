...
/**
     * @return OK
     */
updateReviewFlagStatus(flagId: string, body: UpdateReviewFlagStatusRequest): Promise<void> {
    let url_ = this.baseUrl + "/api/admin/reviews/flags/{flagId}";
    if (flagId === undefined || flagId === null)
        throw new Error("The parameter 'flagId' must be defined.");
    url_ = url_.replace("{flagId}", encodeURIComponent("" + flagId));
    url_ = url_.replace(/[?&]$/, "");
    ...
    return this.http.fetch(url_, options_).then((_response: Response) => {
        return this.processUpdateReviewFlagStatus(_response);
    });
}

protected processUpdateReviewFlagStatus(response: Response): Promise<void> {
    const status = response.status;
    let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
    if (status === 200) {
        return response.text().then((_responseText) => {
...
export class UpdateReviewFlagStatusRequest implements IUpdateReviewFlagStatusRequest {
    status!: string;
    adminNote?: string | undefined;

    constructor(data?: IUpdateReviewFlagStatusRequest) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): UpdateReviewFlagStatusRequest {
        data = typeof data === 'object' ? data : {};
        let result = new UpdateReviewFlagStatusRequest();
        result.init(data);
        return result;
    }
...
export interface IUpdateReviewFlagStatusRequest {
    status: string;
    adminNote?: string | undefined;
}
